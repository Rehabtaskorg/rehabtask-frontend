...
export default function BrowseRequestsPage() {
    ...
    const { data, isLoading } = usePublicRequests(params);
                    ...
                    <div className="max-w-6xl mx-auto">
                        ...
                        <div className="flex flex-col md:flex-row gap-3 bg-white p-2 rounded-xl border border-gray-200 shadow-sm">
                                ...
                                {[{ key: "all", label: "All" }, { key: "pt", label: "PT" }, { key: "ot", label: "OT" }, { key: "slp", label: "SLP" }].map((d) => (
                                    <button
                                        ...
                                        className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${
                                            activeDiscipline === d.key ? "bg-[#137fec] text-white" : "text-gray-500 hover:bg-gray-100"
                                        }`}
                                    >
                                        {d.label}
                                    </button>
                            ...
                            <div className="flex-1 relative">
                                ...
                                <input type="text" value={locationQuery} onChange={(e) => setLocationQuery(e.target.value)} placeholder="Location (City or Zip)" className="w-full bg-gray-50 border-none focus:ring-2 focus:ring-[#137fec]/20 rounded-lg pl-11 pr-4 py-3 text-gray-900 placeholder:text-gray-400 text-sm" />
                            </div>
                            <div className="flex-1 relative">
                                <MdSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                                <input type="text" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Search conditions, keywords..." className="w-full bg-gray-50 border-none focus:ring-2 focus:ring-[#137fec]/20 rounded-lg pl-11 pr-4 py-3 text-gray-900 placeholder:text-gray-400 text-sm" />
                            </div>
                            <button className="bg-[#137fec] text-white font-semibold px-8 py-3 rounded-lg hover:bg-[#137fec]/90 transition-colors text-sm shrink-0">Find Jobs</button>
                        </div>
                        ...
                        <div className="lg:col-span-8 space-y-5">
                            ...
                            {pagination.totalPages > 1 && (
                                <div className="flex items-center justify-between pt-8 border-t border-gray-100">
                                    <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-[#137fec] disabled:opacity-30 transition-colors">
                                        <MdChevronLeft className="text-lg" /> Previous
                                    </button>
                                    <div className="flex gap-2">
                                        {Array.from({ length: Math.min(pagination.totalPages, 5) }, (_, i) => i + 1).map((page) => (
                                            <button key={page} onClick={() => setCurrentPage(page)} className={`w-10 h-10 rounded-lg text-sm font-bold transition-colors ${currentPage === page ? "bg-[#137fec] text-white" : "bg-gray-50 text-gray-500 hover:bg-gray-100"}`}>
                                                {page}
                                            </button>
                                        ))}
                                    </div>
                                    <button onClick={() => setCurrentPage((p) => Math.min(pagination.totalPages, p + 1))} disabled={currentPage === pagination.totalPages} className="flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-[#137fec] disabled:opacity-30 transition-colors">
                        ...
                        <aside className="lg:col-span-4 space-y-6">
                            <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="bg-gray-50 border border-gray-200 rounded-xl p-7 relative overflow-hidden group">
                                <div className="relative z-10">
                                    <div className="bg-[#137fec]/10 w-12 h-12 rounded-lg flex items-center justify-center mb-5"><MdWork className="text-[#137fec] text-2xl" /></div>
                                    ...
                                    <Link href="/register/therapist" className="block w-full text-center bg-[#137fec] text-white font-semibold py-3.5 rounded-xl hover:bg-[#137fec]/90 transition-colors text-sm">Complete Your Profile</Link>
                                </div>
                            ...
                            <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }} className="bg-white border border-gray-200 rounded-xl p-7">
                                <h3 className="text-base font-bold text-gray-900 mb-5 flex items-center gap-2"><MdInfo className="text-[#137fec] text-lg" /> How It Works</h3>
                                <div className="space-y-5">
                                    {[{ step: "1", title: "Browse Listings", desc: "View therapy needs in your area from top health agencies." }, { step: "2", title: "Verify Credentials", desc: "One-time background and license check for security." }, { step: "3", title: "Send Offers", desc: "Apply to cases and coordinate directly with agencies." }].map((item) => (
                                        <div key={item.step} className="flex gap-4">
                                            <div className="shrink-0 w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-xs font-bold text-[#137fec]">{item.step}</div>
                                ...
                                <div className="mt-6 pt-5 border-t border-gray-100">
                                    <Link href="/register/therapist" className="text-[#137fec] text-xs font-semibold flex items-center justify-center gap-1 hover:underline">Learn more about RehabTask <MdArrowForward className="text-sm" /></Link>
                                </div>
                            </motion.div>
                        </aside>
                    </div>
