...
export default function BookingsPreview() {
    return (
        <div aria-hidden="true">
            ...
            {/* Filter tabs */}
            <div className="flex gap-2 px-6 py-3 border-b border-border-light ">
                {FILTER_TABS.map((tab) => (
                    <div
                        key={tab.label}
                        className={`h-8 px-3 rounded-full flex items-center ${tab.active ? "bg-primary/20" : "bg-slate-100 "}`}
                    >
                        <div className={`h-3 w-16 ${tab.active ? "bg-primary/30" : "bg-slate-200 "} rounded`} />
                    </div>
                ))}
            </div>
            ...
            <div className="px-6 py-2">
                ...
                {/* Pagination */}
                <div className="flex items-center justify-between py-4 border-t border-border-light  mt-2">
